		...
		if (request.method == 'GET') {
			...
			if (pathname == '/redeploy') {
				...
				if (query.name) {
					...
					if (!services.length) return sendJSON(response, { success: false, error: 'no services avialable' });

					services.forEach(service => {
						if (service.serviceName == query.name) id_list.push(service.id);
					});
				}
				...
				if (redeployed === 0) return sendJSON(response, { success: false, error: 'Redeloyment failed' });

				if (redeployed < id_list.length)
					return sendJSON(response, {
						success: false,
						error: `Not every service was redeployed ${redeployed}/${id_list.length}`,
					});
